...
const Content: React.FC = () => {
  ...
  useEffect(() => {
    if (documents && !error) {
      dispatch({ type: 'empenhos', value: documents.length })
    }
  ...
  return (
    <Container isLoaded={documents !== undefined}>
      {documents?.map((props) => (
        <Item key={props.NoEmpenho} {...props} />
      ))}
    </Container>
  )
}
...
export default function Documentos () {
  ...
  const { data, error, revalidate } = useFetch<Array<APIProps>>('/wsConsEmpPendExt.rule?sys=LEG', {
